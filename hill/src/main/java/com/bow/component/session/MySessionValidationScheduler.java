...
public class MySessionValidationScheduler implements SessionValidationScheduler, Runnable {
    ...
    public void run() {
        ...
        //分页获取会话并验证
        String sql = "select session_str from shiro_sessions rownum>=? and rownum<?";
        int start = 1; //起始记录
        int size = 20; //每页大小
        List<String> sessionList = jdbcTemplate.queryForList(sql, String.class, start, start+size);
        while(sessionList.size() > 0) {
            ...
            start = start + size;
            sessionList = jdbcTemplate.queryForList(sql, String.class, start, start+size);
        }
